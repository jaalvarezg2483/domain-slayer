...
type Props = {
  ...
  disabled?: boolean;
  /** Solo hora 0–23; minutos siempre 0; un toque en el círculo y se cierra (p. ej. chequeo diario proximidad). */
  hourOnly?: boolean;
};
...
export function ScheduleTimePicker({ hour, minute, onChange, disabled, hourOnly }: Props) {
  ...
  return (
    <div className="schedule-time-picker" ref={wrapRef}>
      ...
      >
        <svg className="schedule-time-picker__icon" viewBox="0 0 24 24" aria-hidden="true">
      ...
      {open ? (
        <div
          ...
          id={`${id}-popover`}
          role="dialog"
          aria-label={hourOnly ? "Hora del día (minuto 0)" : "Hora de ejecución"}
        >
          {!hourOnly ? (
          ...
          >
            ...
            <line
              x1={CX}
              y1={CY}
              x2={CX + (hourOnly || tab === "hour" ? 48 : 56) * Math.cos(handAngle)}
              y2={CY + (hourOnly || tab === "hour" ? 48 : 56) * Math.sin(handAngle)}
          ...
          <p className="schedule-time-picker__hint muted small">
            {hourOnly
              ? "Toque la hora en el círculo (0–23). El envío diario usa siempre el minuto 0."
              : tab === "hour"
